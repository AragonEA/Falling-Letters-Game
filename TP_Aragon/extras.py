...
def dibujar(screen, candidata, listaIzq, listaMedio, listaDer, posicionesIzq ,
    ...
    for i in range(len(listaIzq)):
        screen.blit(defaultFont.render(listaIzq[i], 1, COLOR_COLUMNA_IZQ), posicionesIzq[i]) # Cambia el color de las letras de la columna izquierda a amarillo
    for i in range(len(listaMedio)):
        screen.blit(defaultFont.render(listaMedio[i], 1, COLOR_COLUMNA_MEDIO), posicionesMedio[i])  # Cambia el color de las letras de la columna del medio a naranja
    for i in range(len(listaDer)):
        screen.blit(defaultFont.render(listaDer[i], 1, COLOR_COLUMNA_DER), posicionesDer[i])  # Cambia el color de las letras de la columna derecha a magenta
